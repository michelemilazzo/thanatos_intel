...
def generate_checklists(case_category,case_name):
    ...
    return {'case':case_name,'category':case_category,'created_items':len(items),'items':items,'next_step':'generate_document_requests'}

def generate_document_requests(case_name,items):
    requests=[]
    for item in items:
        requests.append({'document':item,'status':'Pending'})
    return {'case':case_name,'created_requests':len(requests),'requests':requests,'next_step':'notifications'}
