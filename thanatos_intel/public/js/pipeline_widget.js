...
window.ThanatosPipeline = (function () {
    ...
    const STATUS_COLOR = {
        done: '#2ecc71', current: '#C8A96E', pending: 'var(--text-muted)', blocked: '#e74c3c'
    };
...
<div id="${wrapper_id}" style="
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
    ...
    <span style="color:var(--text-muted);font-size:11px">${done_count}/${steps.length} step completati</span>
  </div>
  <div style="background:var(--control-bg);height:4px;border-radius:2px;margin-bottom:18px">
...
<div style="display:flex;gap:14px;align-items:flex-start;padding:10px 0;
     border-bottom:1px solid var(--border-color);${is_current ? 'background:rgba(200,169,110,.08);margin:0 -12px;padding:10px 12px;border-radius:4px' : ''}">
  ...
  <div style="flex:1;min-width:0">
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
      <span style="color:${s.status === 'done' ? 'var(--text-muted)' : 'var(--text-color)'};font-size:13px;
    ...
    ${is_current && s.description ? `<div style="color:var(--text-muted);font-size:11px;margin-top:3px">${s.description}</div>` : ''}
