...
def settle_client(client, start, end, terms_label, due_date):
    """Crea la fattura bozza del consumato [start, end] e azzera il fido. Idempotente per periodo."""
    ...
    inv = frappe.get_doc({
        ...
        "items": [{"item_code": item, "qty": 1, "rate": flt(spent),
                   "description": "Servizi automatizzati — %s" % terms_label}],
        "terms": ("Consumo servizi automatizzati del periodo %s.\n"
